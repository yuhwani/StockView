...
## 🔔 알림 기준 — 어떤 종목이 텔레그램으로 오나

관심·보유 종목을 **3분 주기**로 점검해, 아래 '재료'가 생기면 위 판단 신호·근거와 함께 발송합니다.

- **트리거**: 급등락(±6%, 조정 가능) · DART 공시(호재/악재) · 강한 재료 뉴스
- **매수 추천 위주** *(buy_focus)*: 급등은 **매수 신호 + 거래량 동반**일 때만, 급락은 **매도 신호**일 때만.
  (공시·뉴스 재료는 초기 catalyst라 항상 발송) → "살/팔 만한 것만" 와서 노이즈↓
- **같은 종목 하루 1회** — 단, 알림 후 **추가 ±5% 변동 · 새 공시 · 신호 반전** 시 `🔁 후속`으로 한 번 더(하루 3회 한도)
- **관심목록 외 발굴**: 등록 안 한 종목이라도 **+8% 급등 + '매수 우위'(강한 신호) + 거래량 동반**이면 AI 분석과 함께 발굴 알림 (ETF 제외)
- **오늘의 추천 1등**: 매일(평일) 장 마감 후 전 종목 스캔 → 1등을 텔레그램으로

모든 임계·옵션은 화면(`/settings`)에서 조정할 수 있고, 워커가 다음 주기에 즉시 반영합니다.
